...
const { gitLikeDiffFromDocs } = require("../utils/diffUtils");
...
exports.getDiff = async (req, res) => {
  try {
    ...
    if (!title) {
      return res.status(400).json({ error: "title 쿼리 파라미터가 필요합니다." });
    }
    ...
    const { ops, summary } = gitLikeDiffFromDocs(leftDoc, rightDoc, {
      withIntraLine: false,
      context: Number.isFinite(parseInt(context, 10)) ? parseInt(context, 10) : 3,
    });
